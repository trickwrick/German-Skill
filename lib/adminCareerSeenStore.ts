...
import { getCareerApplications } from "./careerApplicationStore";
import { getMongoClient } from "./mongodb";

const DB_NAME = "germanskill";
const SETTINGS_COLLECTION = "admin_settings";
const SETTINGS_ID = "careers_seen";
const STORE_DIR = path.join(process.cwd(), "data");
const STORE_FILE = path.join(STORE_DIR, "admin-careers-seen.json");
...
export async function getCareersLastSeenAt(): Promise<string | null> {
  const store = await getSeenStore();
  return store.lastSeenAt;
}

export async function markCareersAsSeen() {
  await saveSeenStore({ lastSeenAt: new Date().toISOString() });
}

export function filterUnseenCareerApplications(
...
export async function getUnseenCareerApplications(): Promise<CareerApplication[]> {
  const applications = await getCareerApplications();
  const lastSeenAt = await getCareersLastSeenAt();
  return filterUnseenCareerApplications(applications, lastSeenAt);
}

export async function getUnseenCareerCount(applications: CareerApplication[]) {
  const lastSeenAt = await getCareersLastSeenAt();
  return filterUnseenCareerApplications(applications, lastSeenAt).length;
}
